...
const STATE = {
    ...
    favorites: [],
    favoritesLiveAQI: {}, // Cache of live AQI data for favorite cities
    trendChart: null,
    compareChart: null,
    leafletMap: null,
    mapMarker: null,
...
function getAQIUrl(lat, lon) {
    const token = localStorage.getItem("waqi-token") || "";
    return `/api/air-quality?latitude=${lat}&longitude=${lon}&token=${encodeURIComponent(token)}`;
}
...
function initTokenSettings() {
    ...
    if (saveBtn && tokenInput) {
        saveBtn.addEventListener("click", () => {
            ...
            localStorage.setItem("waqi-token", val);
...
function loadCityData(city) {
    ...
    fetch(getAQIUrl(city.latitude, city.longitude))
        .then(res => res.json())
        .then(data => {
...
function renderDashboard(city, data) {
    ...
    // Use WAQI ground-station data if available, fallback to Open-Meteo model estimations
    const isWaqi = data.waqi_aqi !== undefined && data.waqi_aqi !== null;
    const aqi = Math.round(isWaqi ? data.waqi_aqi : data.current_aqi.us_aqi);
    ...
    if (isWaqi && data.station_name) {
        aqiDescElement.innerHTML = `Real-time ground station measurements from <b>${data.station_name}</b>.`;
    } else {
        aqiDescElement.textContent = `Copernicus forecast model estimation: ${aqiInfo.desc}`;
    }
    ...
    // Upgrades: Generate weather correlation insights and WHO limit warning list
    generateWeatherInsights(weather, data.current_aqi);
    generateWHOAlerts(data.current_aqi);
...
/**
 * Calculates and displays weather conditions relative to local pollution concentrations.
 */
function generateWeatherInsights(weather, aqiData) {
    ...
    // Wind factor
    if (windSpeed > 15) {
        message += `Strong winds of ${Math.round(windSpeed)} km/h are actively dispersing atmospheric pollutants, helping dilute surface-level concentrations. `;
    } else if (windSpeed < 6) {
        message += `Stagnant local wind speeds (${Math.round(windSpeed)} km/h) are limiting atmospheric ventilation, trapping pollutants near ground level. `;
    } else {
        message += `Moderate wind speeds of ${Math.round(windSpeed)} km/h are supporting normal pollutant dispersion patterns. `;
    }
    
    // Photochemical factor (Heat & sunlight builds ozone)
    if (temp > 30 && aqi > 50) {
        message += `High ambient temperatures (${Math.round(temp)}°C) accelerate photochemical reactions, which can lead to increased ground-level Ozone (O3) buildup during daylight hours. `;
    }
    
    // Humidity factor (Moisture traps particulates)
    if (humidity > 75) {
        message += `High relative humidity (${Math.round(humidity)}%) contributes to atmospheric moisture retention, causing particulate matter (PM2.5 and PM10) to cluster and settle closer to the ground. `;
    }
    
    // Dry factor (Dust lift)
    if (humidity < 35 && windSpeed > 12) {
        message += `Dry weather conditions (${Math.round(humidity)}% humidity) combined with active winds are conducive to lifting mineral dust and dry soil particles into the air. `;
    }
    
    // Conclusion remarks
    if (aqi <= 50) {
        message += "Overall atmospheric conditions are healthy with low concentration rates.";
    } else if (aqi > 150) {
        message += "Stagnation index is critical. Relieve atmospheric exposure by staying indoors.";
    }
    
    document.getElementById("weather-correlation-text").textContent = message;
}

/**
 * Calculates and lists warnings for any pollutants exceeding standard WHO daily safety limits.
 */
function generateWHOAlerts(aqiData) {
    const list = document.getElementById("who-alerts-list");
    list.innerHTML = ""; // Clear list
    
    const exceedances = [];
    
    const pollutantsList = [
        { name: 'PM2.5', key: 'pm2_5', limit: WHO_GUIDELINES.pm2_5, unit: 'µg/m³' },
        { name: 'PM10', key: 'pm10', limit: WHO_GUIDELINES.pm10, unit: 'µg/m³' },
        { name: 'NO2', key: 'nitrogen_dioxide', limit: WHO_GUIDELINES.no2, unit: 'µg/m³' },
        { name: 'Ozone (O3)', key: 'ozone', limit: WHO_GUIDELINES.o3, unit: 'µg/m³' },
        { name: 'Carbon Monoxide (CO)', key: 'carbon_monoxide', limit: WHO_GUIDELINES.co, unit: 'mg/m³' },
        { name: 'Sulphur Dioxide (SO2)', key: 'sulphur_dioxide', limit: WHO_GUIDELINES.so2, unit: 'µg/m³' }
    ];
    
    pollutantsList.forEach(p => {
        let val = aqiData[p.key];
        if (val !== undefined && val !== null) {
            // CO unit conversion: Open-Meteo returns CO in ug/m3, WHO safe limit is in mg/m3.
            // 1 mg/m3 = 1000 ug/m3. So we divide the Open-Meteo CO value by 1000 before comparing.
            if (p.key === 'carbon_monoxide') {
                val = val / 1000;
            }
            
            if (val > p.limit) {
                const multiplier = (val / p.limit).toFixed(1);
                exceedances.push({
                    name: p.name,
                    val: p.key === 'carbon_monoxide' ? val.toFixed(2) : Math.round(val),
                    limit: p.limit,
                    unit: p.unit,
    ...
    if (exceedances.length === 0) {
        list.innerHTML = `
            <div class="who-alert-safe">
                <i data-lucide="shield-check" class="who-safe-icon"></i>
                <span>All key pollutants are within WHO daily guidelines. Clean air!</span>
            </div>
        `;
    } else {
        exceedances.forEach(e => {
            const alertItem = document.createElement("div");
            alertItem.className = "who-alert-item";
            alertItem.innerHTML = `
                <i data-lucide="alert-octagon" class="who-alert-icon"></i>
                <span><b>${e.name}</b> is <b>${e.multiplier}x</b> above the WHO daily limit (${e.val} vs limit of ${e.limit} ${e.unit}).</span>
            `;
...
async function fetchFavorites() {
    try {
        STATE.favoritesLiveAQI = {}; // Reset cache on fetch
        updateLeaderboard(); // Reset display
    ...
    fetch(getAQIUrl(lat, lon))
        ...
        .then(data => {
            ...
            if (valEl && badgeEl && data.current_aqi) {
                ...
                // Cache the AQI results for leaderboard compilation
                const favCity = STATE.favorites.find(f => f.id === id);
                if (favCity) {
                    STATE.favoritesLiveAQI[id] = {
                        name: favCity.city_name,
                        country: favCity.country,
                        region: favCity.region,
                        aqi: aqi,
                        info: info
                    };
                    updateLeaderboard();
...
/**
 * Compiles and ranks the active favorites cache from cleanest to most polluted.
 */
function updateLeaderboard() {
    ...
    const resolvedFavs = Object.values(STATE.favoritesLiveAQI);
    ...
    // Sort from cleanest (lowest US AQI) to most polluted (highest)
    resolvedFavs.sort((a, b) => a.aqi - b.aqi);
    
    card.classList.remove("hidden");
    list.innerHTML = "";
    
    resolvedFavs.forEach((fav, index) => {
        const rank = index + 1;
        ...
        item.innerHTML = `
            <div class="leaderboard-left">
                ...
                <div class="leaderboard-city-title">
                    <span class="leaderboard-name">${fav.name}</span>
                    <span class="leaderboard-country">${regionText}${fav.country}</span>
                </div>
            </div>
            <div class="leaderboard-right">
                <span class="leaderboard-aqi-badge" style="background-color: ${fav.info.color}">AQI: ${fav.aqi}</span>
            </div>
        `;
...
async function runComparison(cityA, cityB) {
    ...
    try {
        const [resA, resB] = await Promise.all([
            fetch(getAQIUrl(cityA.latitude, cityA.longitude)),
            fetch(getAQIUrl(cityB.latitude, cityB.longitude))
        ]);
